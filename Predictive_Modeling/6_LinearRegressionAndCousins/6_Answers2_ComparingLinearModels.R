...
set.seed(0)
lm_model = train( fingerprints_training, permeability_training,
                  method="lm", 
                  preProcess=c("center","scale"),
                  trControl=trainControl(method="repeatedcv",repeats=5) )
lm_model$times
...
# 
set.seed(0)
rlm_model = train( fingerprints_training, permeability_training, method="rlm",
                   preProcess=c("pca"),
                   trControl=trainControl(method="repeatedcv",repeats=5) )

saveRDS(rlm_model, 'rlmmodel_ans_6.3.rds')
rlm_model$times
