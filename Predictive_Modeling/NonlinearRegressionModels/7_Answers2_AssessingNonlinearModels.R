...
saveRDS(nnetModel, 'NN_ans_7.2.rds')

#w/tune
nnGrid = expand.grid(.decay=c(0,0.01,0.1),
                     .size=1:10)

tune_nnetModel = train(x=trainingData$x, 
                  y=trainingData$y, 
                  method="nnet",
                  preProc=c("center", "scale"),
                  linout=TRUE,
                  ...
                  MaxNWts=10 * (ncol(trainingData$x)+1) + 10 + 1,
                  maxit=500)
saveRDS(tune_nnetModel, 'tunedNN_ans_7.2.rds')

tune_nnetPred = predict(tune_nnetModel, newdata=testData$x)
tune_nnetPR = postResample(pred=tune_nnetPred, obs=testData$y)
rmses = c(rmses,tune_nnetPR[1])
r2s = c(r2s,tune_nnetPR[2])
methods = c(methods,"NN_tune")
...
saveRDS(avNNetModel, 'avNet_ans_7.2.rds')
...
saveRDS(marsModel, 'marsModel_ans_7.2.rds')
